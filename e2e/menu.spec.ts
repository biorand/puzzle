...
test.describe('Puzzle Menu Grid', () => {
  ...
  test('renders all 5 puzzle cards in the grid', async ({ page }) => {
  ...
  test('unlocking a puzzle reveals its card as enabled', async ({ page }) => {
    // Simulate completing keypad by setting its score
    await page.evaluate(() => {
      localStorage.setItem('repuzzles-keypad-score', '1');
      localStorage.setItem('repuzzles-unlock', JSON.stringify({ tutorialStep: 5, unlockedAll: false }));
    });
    await page.reload();
    await page.waitForSelector('#menu-grid');
